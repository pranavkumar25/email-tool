...
export default function ReportsPage() {
  return (
    <div className="space-y-6">
      <PageHeader
        title="Reports"
        subtitle="Performance across campaigns, audience, and revenue."
        actions={
          <>
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Stat label="Emails sent" value={compactNumber(reportTotals.sent)} icon={PaperAirplaneIcon} delta={{ value: "8.2%", dir: "up" }} accent />
        <Stat label="Open rate" value={`${reportTotals.openRate}%`} icon={EnvelopeOpenIcon} delta={{ value: "1.4%", dir: "up" }} />
        <Stat label="Click rate" value={`${reportTotals.clickRate}%`} icon={CursorArrowRaysIcon} delta={{ value: "0.6%", dir: "down" }} />
        <Stat label="Revenue" value={`$${compactNumber(reportTotals.revenue)}`} icon={BanknotesIcon} delta={{ value: "12.5%", dir: "up" }} />
      </section>
        ...
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-4">
            {deviceBreakdown.map((d) => (
              <MeterRow key={d.label} label={d.label} value={d.value} tone="accent" suffix="%" />
            ))}
          </div>
        </Card>
        <Card className="p-5">
          ...
          <div className="mt-4 space-y-4">
            {locationBreakdown.map((l) => (
              <MeterRow key={l.label} label={l.label} value={l.value} tone="info" suffix="%" />
            ))}
          </div>
        </Card>
      </div>
